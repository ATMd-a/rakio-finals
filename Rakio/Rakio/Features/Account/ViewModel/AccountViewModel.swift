        ...
        do {
            ...
            try await withThrowingTaskGroup(of: [WatchedItem].self) { group in
                ...
                for seriesDoc in seriesSnapshot.documents {
                    ...
                    // Decode Series (using inline decoding for simplicity as shown below)
                        ...
                        for epDoc in epSnapshot.documents {
                            ...
                            for code in codes {
                                if let watchedEntry = watchedVideoData.first(where: { $0.code == code }) {
                                    ...
                                    // 💡 FIX: Calculate the required fields
                                    // Use the official ThumbnailData struct from your utility file let thumbnailData = ThumbnailData.generate(for: code)
                                    let thumbnailData = ThumbnailData.generate(for: code)
                                    let videoSource = thumbnailData.source

                                    let progress = watchHistory[code]?["progress"] as? Double ?? 0.0 // Fetch progress here using the key 'code'

                                    let item = WatchedItem(
                                        series: series,
                                        episode: episode,
                                        lastWatchedAt: watchedEntry.lastWatched,
                                        watchedVideoID: code, // This is the video ID/code
                                        source: videoSource, // This is the inferred source
                                        progress: progress // Add progress from the history map
                                    )
